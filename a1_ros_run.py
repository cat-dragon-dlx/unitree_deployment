...
def main(args):
    ...
    if args.mode == "jetson":
        ...
        def policy(obs):
            ...
            recurrent_embedding = memory_module(obs)
            # print("recurrent_embedding.shape=", recurrent_embedding.shape)
            actions = actor_mlp(recurrent_embedding)
            return actions
        ...
        with torch.no_grad():
            ...
            while not rospy.is_shutdown():
                # print("unitree_real_env.is_skill_mode()=", unitree_real_env.is_skill_mode())
                # print("using_walk_policy=", using_walk_policy)
                ...
                timestamp = time.time()
                with open("actions_log.csv", "a") as f:
                    f.write(f"{timestamp},")
                    np.savetxt(f, np_actions, delimiter=",")
