...
def main(args):
    ...
    #创建depth_image文件保存的地方
    current_date = datetime.datetime.now().strftime("%Y-%m-%d-%H_%M_%S")  # Use timestamp to avoid overwriting files
    save_dir = f"./depth_images/{current_date}"  # Change this path to your desired directory
    print(save_dir)
    if not os.path.exists(save_dir):
        print(f"make depth_images directory:{save_dir}")
    ...
    try:
        ...
        while not rospy.is_shutdown():
            ...
            # Save depth image to the specified folder using Pillow
            timestamp = rospy.Time.now().to_nsec()  # Use timestamp to avoid overwriting files
            depth_image_filename = os.path.join(save_dir, f"depth_image_{timestamp}.png")
            # Save depth image to the specified folder
            depth_image_8bit = np.clip(depth_image_, 0, 255).astype(np.uint8)  # Normalize to 0-255 for saving
            pil_image = PILImage.fromarray(depth_image_8bit)  # Convert NumPy array to Pillow Image
            pil_image.save(depth_image_filename)  # Save the image
